...
import { Github_API } from '../src/app/modules/github-api';
import {getSessionId} from '../src/app/modules/github-session';
...
describe('Github_API', () => {
    ...
    test('should find markdown files', async () => {
        nock('https://api.github.com')
            .get(`/repos/${owner}/${repo}/git/trees/${api.repo_details.default_branch}?recursive=0`)
            .reply(200, {
                tree: [
    ...
    test('should handle errors when fetching markdown files', async () => {
        ...
        await expect(api.get_md_files()).rejects.toThrow('Github API is unable to traverse through the repository : 500');
    });
    ...
    test('should create a pull request', async () => {
        nock('https://api.github.com')
            ...
            .reply(201, { html_url: 'https://github.com/test-owner/test-repo/pull/1' });
